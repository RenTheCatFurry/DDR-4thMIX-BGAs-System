...
local load_song_bg = not first_texture.img
...
local function load_sprite(i, tex)
    ...
    local tex_to_load
    if not load_song_bg then tex_to_load = tex.img end
    ...
    return Def.Sprite{
        ...
        BeatMessageCommand = function(self, params)
            ...
            if frames_beat then
                -- frames config
                if frames_seq then
                    self:setstate((i + beat / delay) % #states)
                ...
                elseif frames_anim then
                    ...
                    self:setstate((beat / delay) / (iterator / (num_sprites * 2) + 1) % #states)
                
                else
                    self:setstate(beat / delay % #states)
                end
            end
...
local layer = Def.ActorFrame{
    ...
    OnCommand = function(self)
        ...
        -- fade config
        if fade_stop then fade_linear = true end
        ...
        self:SetUpdateFunction(function(self)
            ...
            if fade_length then
                ...
                if fade_linear then
                    ...
                    -- apply fade
                    if fade_rgb then
                        self:diffuse(beat_linear, beat_linear, beat_linear, sec_blink)
                    else
                        self:diffuse(sec_blink, sec_blink, sec_blink, beat_linear)
                    end
                ...
                -- animation fade normal
                    ...
                    -- apply fade
                    if fade_rgb then
                        self:diffuse(opacity_value, opacity_value, opacity_value, sec_blink)
                    else
                        self:diffuse(sec_blink, sec_blink, sec_blink, opacity_value)
                    end
                end
            end
        end)
            ...
            self:SetUpdateFunction(function(self)
                ...
                elseif snail_effect then
                    ...
                    if effect == "snailinreverse" or effect == "snailoutreverse" then
                        self:x(snail_x_coord[#snail_x_coord - i] * sprite_size.w)
                        self:y(snail_y_coord[#snail_y_coord - i] * sprite_size.h)
                    else
                        self:x(snail_x_coord[i + 1] * sprite_size.w)
                    ...
                    if effect == "snailin" or effect == "snailinreverse" then
                        alpha_anim = math.abs(
                            math.floor(((beat / effect_length) + (num_sprites - (i / num_sprites))) % 2) - 1
                        )
                    else
                        alpha_anim = math.floor(
                            ((beat / effect_length) + (num_sprites - (i / num_sprites))) % 2
                        )
                    end
                ...
                elseif misc_effect then
                    ...
                    elseif effect == "scrollxy" then
                        ...
                        beat = (-0.5 + length / 2 + (beat / ((i / num_sprites) + 1))) % length
